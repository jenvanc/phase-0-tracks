...
# gives the path for the required file, shorter than require

class VirusPredictor
#Initialize a new instance of the class with state_of_origin, population density and population as arguments
  def initialize(state_of_origin, population_density, population)
    @state = state_of_origin
    @population = population
    @population_density = population_density
  end
#calls other methods, returns speed_of_spread but also prints virus_effects and speed_of_spread
  def virus_effects
    predicted_deaths
    speed_of_spread
  end
  ...
  private
#predicts deaths based on population density and rounds down to nearest integer
...
#based on pop density, increases speed by .5 month
...
STATE_DATA.each do |state, hash|
  state_instance = VirusPredictor.new(state, STATE_DATA[state][:population_density], STATE_DATA[state][:population])
